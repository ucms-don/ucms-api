        ...
        public async Task<(List<object>? Data, bool Forbidden)> HandleAsync(Query q, CancellationToken ct)
        {
            ...
            var estimates = await db.Estimates
                ...
                .Select(e => (object)new
                {
                    ...
                    VatAmount     = Math.Round(e.Sections.SelectMany(s => s.EstimateItems).Sum(i => (i.Volume * i.ClientUnitPrice + i.Volume * i.MaterialUnitPrice) * i.VatRate / 100m), 2),
                    TotalWithVat  = Math.Round(e.Sections.SelectMany(s => s.EstimateItems).Sum(i => (i.Volume * i.ClientUnitPrice + i.Volume * i.MaterialUnitPrice) * (1m + i.VatRate / 100m)), 2),
